...
class Theater_model extends CI_Model {
	...
	var $table = 'theater_tb';

        var $column_order = array('FilmId','FilmName','	TheaterName','Place','ShowDate','ShowTime','CreatedOn','Status'); //set column field database for datatable orderable
	var $column_search = array('FilmName','Status'); //set column field database for datatable searchable.
 ...
 function getDataTables($releaseid)
	{

	   $this->getDataTableQuery($releaseid);
 ...
 private function getDataTableQuery($releaseid)
	{
		
		$this->db->from($this->table);

		$this->db->where("FilmId =",$releaseid);
		$i = 0;

...
if (isset($CI->session)) {
	$adminUserType = $CI->session->userdata('ADMIN_USER_TYPE');
}

if (($adminUserType === FALSE || $adminUserType === '' || is_null($adminUserType)) && isset($_SESSION['ADMIN_USER_TYPE'])) {
	$adminUserType = $_SESSION['ADMIN_USER_TYPE'];
}
?>

<?php if ($adminUserType != "SUPER_ADMIN"): ?>
	<?php
	...
	foreach ($admin_navigation  as $row):
		...
		if ($mainMenuId != $row["Id"]):
			...
			if ($row["SubMenuItem"] == "NULL"):
				$db = new Database();
				$where = "MenuItemId='{$mainMenuId}' AND UserGroup='" . $adminUserType . "'";
...
<?php elseif ($adminUserType == "SUPER_ADMIN"): ?>
